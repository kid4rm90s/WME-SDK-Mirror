...
This is a **documentation mirror and processing system** for the Waze Map Editor (WME) SDK. The project maintains a local copy of the WME SDK TypeDoc documentation and provides a 7-step Python pipeline to:
...
5. Keep the WME SDK skill in sync with the latest SDK documentation
...
| `copy-to-source.py` | Sync fresh `.md` files from `scripts/` to `../source/` for version control | Updated `.md` files in `source/` with current timestamps |
...
```powershell
Push-Location production/latest/scripts
py cleanup.py; py download-pages.py --force; py extract-to-md.py; py copy-to-source.py; py create-grouped-md-files.py; py update-skill.py
Pop-Location
```
...
py copy-to-source.py            # sync to version control
